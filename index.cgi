...
print '<li>' . "\n";
print '<a href="#hide"><i class="fa fa-bars fa-fw"></i> <span>隐藏菜单</span></a>' . "\n";
print '</li>' . "\n";
...
print '<ul class="navigation">' . "\n";
if ($gconfig{'log'} && &foreign_available("webminlog")) {
	print '<li><a target="page" href="webminlog/" onClick="show_logs(); return false;"><i class="fa fa-fw fa-exclamation-triangle"></i> <span>模块日志</span></a></li>' . "\n";
}
print '<li><a target="page" href="/body.cgi"><i class="fa fa-fw fa-info"></i> <span>系统信息</span></a></li>' . "\n";
if (&get_product_name() eq 'webmin' && !$ENV{'ANONYMOUS_USER'} && $gconfig{'nofeedbackcc'} != 2 && $gaccess{'feedback'} && $gconfig{'feedback_to'} || &get_product_name() eq 'usermin' && !$ENV{'ANONYMOUS_USER'} && $gconfig{'feedback'}) {
	print '<li><a target="page" href="feedback_form.cgi"><i class="fa fa-fw fa-envelope"></i> <span>发送反馈</span></a></li>' . "\n";
}
if (&foreign_available("webmin")) {
	print '<li><a target="page" href="webmin/refresh_modules.cgi"><i class="fa fa-fw fa-refresh"></i> <span>重载模块</span></a></li>' . "\n";
}
print '</ul>' . "\n";
print '</aside>' . "\n";
